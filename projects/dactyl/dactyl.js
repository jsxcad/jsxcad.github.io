...
const jscadCylinder = ({ r = 3.3, h = 10, center = true, fn = 50 }) => {
  if (center) {
    return cylinder({ radius: r, height: h, resolution: fn });
  } else {
    throw Error('die');
  }
}

const jscadCube = ({ size, center }) => {
  if (center) {
    return cube(...size);
  } else {
    return cube(...size).right().back();
  }
}

const jscadSphere = ({ r = 1, center = true, fn = 20 }) => {
  if (center === false) {
    throw Error('die');
  } else {
    return sphere({ radius: r, resolution: fn });
  }
}

const mirror = ([x, y, z], shape) => {
  if (!!x + !!y + !!z !== 1) {
    throw Error('die');
  }
  if (x !== 0) {
    return shape.scale([-1, 1, 1]);
  } else if (y !== 0) {
    return shape.scale([1, -1, 1]);
  } else if (z !== 0) {
    return shape.scale([1, 1, -1]);
...
const linear_extrude = extrude;

const rotate = (angle, axes, shape) => {
  if (false && axes[0] + axes[1] + axes[2] !== 1) {
    // FIX: Support rotation around arbitrary axis, e.g., [1, 1, 0].
    throw Error('die');
  }
  if (axes[0] !== 0) {
    return shape.rotateX(angle);
  } else if (axes[1] !== 0) {
    return shape.rotateY(angle);
  } else if (axes[2] !== 0) {
    return shape.rotateZ(angle);
  } else {
    throw Error('die');
  }
}

const quickhull3d = hull;
...
function old_single_plate() {
  const top_wall = translate(
    ...
    jscadCube({size: [keyswitch_width + 3, 1.5, plate_thickness], center: true}));

  const left_wall = translate(
    [1.5/2 + keyswitch_width/2, 0, plate_thickness/2],
    jscadCube({size: [1.5, keyswitch_height + 3, plate_thickness], center: true}));

  const side_nub = quickhull3d(
    translate(
      [1.5/2 + keyswitch_width/2, 0, plate_thickness/2],
      jscadCube({size: [1.5, 2.75, plate_thickness], center: true})),
    translate([keyswitch_width/2, 0, 1],
      rotate(Math.PI/2*180/Math.PI, [1, 0, 0],
        jscadCylinder({r: 1, h: 2.75, center: true, fn: draft_mode ? 10 : 30}))));
...
function single_plate() {
    const top_wall = translate(
        ...
        jscadCube({size: [keyswitch_width + 3, 2.2, plate_thickness], center: true}));
    const left_wall = union(
        ...
        translate([1.5/2 + alps_notch_width/2, 0, plate_thickness - alps_notch_height/2],
            jscadCube({size: [1.5, keyswitch_height + 3, 1.0], center: true})));
...
function web_post() {
    return translate([0, 0, web_thickness/-2 + plate_thickness],
        jscadCube({size: [post_size, post_size, web_thickness], center: true}));
}
...
function double_plates() {
  ...
  let top_plate =
    translate([0, (plate_height + mount_height)/2,
      ...
      jscadCube({size: [mount_width, plate_height, web_thickness], center: true}));

  const stabilizer_cutout = union(
    color([1, 0, 0, 1/2],
      translate([0.5, 12, plate_thickness - web_thickness/2],
        jscadCube({size: [14.2, 3.5, web_thickness], center: true}))),
    color([1, 0, 0, 1/2],
      translate([0.5, 12, plate_thickness - web_thickness/2 - 1.4],
        jscadCube({size: [16, 3.5, web_thickness], center: true}))));
...
function wall_sphere_at(coords) {
  const shape = draft_mode ?
    jscadCube({size: [1, 1, 1], center: true}) :
    jscadSphere({r: 1, center: true, fn: wall_sphere_n});
...
function bottom_key_guard() {
  return translate([0, 0, -web_thickness/2 + -4.5],
    jscadCube({size: [mount_width, mount_height, web_thickness], center: true}));
}

function bottom_front_key_guard() {
  return translate([0, mount_height/4, (-web_thickness/2) + -4.5],
    jscadCube({size: [mount_width, mount_height/2, web_thickness], center: true}));
}
          ...
          bottom_hull(
            place(
              translate([0, 0, stand_radius/-2 + -4.5],
                jscadSphere({r: stand_radius, center: true, fn: draft_mode ? 20 : 30})))),
          place(
            translate([0, 0, stand_radius/-2],
              jscadCube({size: [stand_diameter, stand_diameter, stand_radius], center: true}))),
          bottom(1.5, place(
            translate([0, 0, stand_radius/-2 + -4.5],
                jscadSphere({r: bumper_radius, center: true, fn: draft_mode ? 20 : 30})))));
      };
...
function screw_hole() {
  return translate([0, 0, 3/2],
    jscadCylinder({r: 1.5, h: 60, center: true, fn: wall_sphere_n}));
}
...
function led_hole() {
  return color([0, 0, 1], translate([0, 0, 7/4],
    jscadCylinder({r1: 1.6, r2: 1.5, h: 4, center: true, fn: wall_sphere_n})));
}
...
function circuit_cover(width, length, height) {
  ...
  const cover_sphere =
    jscadSphere({r: cover_sphere_radius, center: true, fn: 20});
...
function trrs_hole() {
  const shape = union(
    jscadCylinder({r: trrs_radius, h: trrs_hole_depth, center: true, fn: 50}),
    translate([0, (trrs_radius + 5)/2, 0],
      jscadCube({size: [trrs_diameter, trrs_radius + 5, trrs_hole_depth], center: true})));
...
function trrs_box_hole() {
  return translate([0, 1, -3.5],
    jscadCube({size: [14, 14, 7], center: true}));
}
  ...
  return color([1, 0, 0],
    key_place(1/2, 3/2,
      translate([0, 0, teensy_pcb_thickness/-2 + -teensy_offset_height],
        jscadCube({size: [18, 30.5, teensy_pcb_thickness], center: true}))));
}
  ...
  return difference(
    union(
      color([0, 1, 0],
        key_place(1/2, 3/2,
          translate([0, 0, -2],
            jscadCube({size: [3, 3, 9], center: true})))),
      quickhull3d(
        color([0, 0, 1],
          key_place(1/2, 2,
            translate([0, 0, -2],
              jscadCube({size: [3, 6, 9], center: true})))),
        color([0, 0, 1],
          key_place(1/2, 3/2,
            translate([0, 30.5/-2,
                -teensy_offset_height /* + (teensy_pcb_thickness + 3)/-2 */],
              jscadCube({size: [3, 3, teensy_pcb_thickness + 3], center: true})))))),
    teensy_pcb(),
    color([1, 0, 0],
      key_place(1/2, 3/2,
...
function usb_cutout() {
  ...
  const side_cylinder = translate([(hole_width - hole_height)/2, 0, 0],
    jscadCylinder({r: side_radius, h: teensy_length, center: true, fn: 20}));
  ...
  return difference(
    union(
      ...
      difference(
        ...
        teensy_cover(),
        trrs_cutout(),
        translate([0, 0, -5], jscadCube({size: [1000, 1000, 10], center: true})),
        screw_holes(params))),
    usb_cutout());
}
...
startClock();
const result = dactylMain({ draft: true, part: 'dactyl_bottom_right' });
console.log(`Compute: ${getClock()}`);
result.writeThreejsPage('dactyl_bottom_right.html');
console.log(`Written: ${getClock()}`);
